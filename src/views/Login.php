...
<main class="flex items-center justify-center p-4">
  <div class="w-full max-w-md p-8 rounded-xl shadow-2xl bg-gray-800/70 backdrop-blur-lg border border-gray-700">
    ...
    <?php if (!empty($data['message'])): ?>
      <div class="mb-6 text-center text-red-400 font-semibold">
        <?= htmlspecialchars($data['message']) ?>
      </div>
    <?php endif; ?>

    <form action="<?= htmlspecialchars($_ENV['BASE_URL'] . '/login') ?>" method="POST" class="space-y-6">
      <div>
    ...
    <div class="mt-8 text-center text-sm text-gray-400">
      ...
      <a href="<?= htmlspecialchars($_ENV['BASE_URL'] . '/register') ?>" class="font-medium text-blue-500 hover:text-blue-400">
        Sign up
      </a>
    </div>
  </div>
